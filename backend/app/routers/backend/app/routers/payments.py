...
Webhooks CinetPay avec vérification HMAC obligatoire.
"""
import hmac
import hashlib
...
# Durée des packs en jours
PACK_DURATION_DAYS = {
    "standard": 90,   # ~3 mois (saison courte)
    "pro": 180,       # ~6 mois (saison longue)
    "coop": 365,      # 1 an (coopérative)
}


def verify_cinetpay_signature(payload: bytes, signature: str) -> bool:
    """Vérifie l'authenticité du webhook CinetPay via HMAC-SHA256."""
    expected = hmac.new(
...
async def cinetpay_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    Sécurisé par HMAC : toute requête non signée est rejetée.
    """
    ...
    # 2. Parser le payload
    data = await request.json()    transaction_id = data.get("transaction_id")
    ...
    # 3. Idempotence : éviter double-crédit si webhook renvoyé
    ...
    # 6. Si paiement réussi → Créer/étendre la licence
    if status_payment == "ACCEPTED" and pack_type in PACK_DURATION_DAYS:
        days = PACK_DURATION_DAYS[pack_type]
        
        # Chercher licence existante active pour étendre sa durée
